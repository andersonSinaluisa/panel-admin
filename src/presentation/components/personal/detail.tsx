import { CATALOGUE_TYPE_COUNTRY, CATALOGUE_TYPE_DOCUMENT, CATALOGUE_TYPE_MAIN_ROL_CLIENT, CATALOGUE_TYPE_ORGANISATION_OR_ENTITY_WITH_PERSONALITY_LEGAL, CATALOGUE_TYPE_RECORD_AVAILABILITY, CATALOGUE_TYPE_STREET, CATALOGUE_TYPE_USER_STATE, SUCCESS_HTTP_CODE_CREATED } from "application/common";
...
const DetailPersonal = (props: DetailPersonalProps) => {
  ...
  useEffect(() => {
    setForm(props.GetPersonalById.data.data)

  }, [props.GetPersonalById]);


  useEffect(() => {
    if (props.UpdatePersonal.status === 200) {
      setMessage({
    ...
    if (props.UpdatePersonal.status !== 0) {
      setMessage({
        description: props.UpdatePersonal.error,
        title: "Error",
        type: "danger",
        visible: true
      })
